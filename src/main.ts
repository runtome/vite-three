...
// new GLTFLoader().load('models/worker@walk.glb', (gltf) => {
...
async function loadEve() {
  ...
  const [eve, idle, run] = await Promise.all([
    loader.loadAsync('models/worker@walk.glb'),
    loader.loadAsync('models/worker@idle.glb'),
    loader.loadAsync('models/worker@run.glb')
  ])
